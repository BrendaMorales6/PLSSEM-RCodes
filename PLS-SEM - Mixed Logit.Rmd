---
title: "RCode SEM-WTP - MixedLogit"
date: "`r Sys.Date()`"
output: pdf_document
---
...
mnlT0 <- mlogit(Choice ~ 0 + Project + Specie + Location + Price + ASC_accept,
               data = df_mldTotal)
summary(mnlT0)

# MNL baseline (atributes + ASC) ---
mnlA0 <- mlogit(Choice ~ 0 + Project + Specie + Location + Price  + ASC_accept,
               data = df_mldA)
summary(mnlA0)

# MNL baseline (atributes + ASC) ---
mnlB0 <- mlogit(Choice ~ 0 + Project + Specie + Location + Price + ASC_accept,
               data = df_mldB)
summary(mnlB0)
```
```{r}
#Price interactuations

df_mldTotal$Price_pyme  <- df_mldTotal$Price * df_mldTotal$pyme
df_mldTotal$ASC_pyme  <- df_mldTotal$ASC_accept * df_mldTotal$pyme
df_mldTotal$Price_large  <- df_mldTotal$Price * df_mldTotal$large
df_mldTotal$ASC_large  <- df_mldTotal$ASC_accept * df_mldTotal$large
df_mldTotal$Price_Segment <- df_mldTotal$Price * df_mldTotal$DSegment
df_mldTotal$ASC_Segment <- df_mldTotal$ASC_accept * df_mldTotal$DSegment

clTotalpyme_asc <- gmnl(
  Choice ~ 0 + Project + Specie + Location + Price + ASC_accept  + ASC_pyme,
  data = df_mldTotal,
  model = "mnl"
)
...
clTotallarge_asc <- gmnl(
  Choice ~ 0 + Project + Specie + Location + Price + ASC_accept  + ASC_large,
  data = df_mldTotal,
  model = "mnl"
)


clTotalpyme_price <- gmnl(
  Choice ~ 0 + Project + Specie + Location + Price + ASC_accept  +  Price_pyme,
  data = df_mldTotal,
  model = "mnl"
)


clTotallarge_price <- gmnl(
  Choice ~ 0 + Project + Specie + Location + Price + ASC_accept  + Price_large,
  data = df_mldTotal,
  model = "mnl"
)


summary(clTotalpyme_asc)
summary(clTotallarge_asc)
summary(clTotalpyme_price)
summary(clTotallarge_price)


```

#### Conditional logit: Production sector interactions
```{r}
df_mldTotal$Price_Servicios <- df_mldTotal$Price * df_mldTotal$Servicios
df_mldTotal$Asc_Servicios <- df_mldTotal$ASC_accept * df_mldTotal$Servicios

clTotalServ_asc <- gmnl(
  Choice ~ 0 + Project + Specie + Location + Price + ASC_accept  + Price_Servicios,
  data = df_mldTotal,
  model = "mnl"
)

clTotalServ_price <- gmnl(
  Choice ~ 0 + Project + Specie + Location + Price + ASC_accept  +  ASC_Servicios,
  data = df_mldTotal,
  model = "mnl"
)

summary(clTotalServ_asc)
summary(clTotalServ_price)
```

#### Conditional logit: TPB + Sociodemographic variables
```{r}
#Price interactuations
df_mldTotal$Price_Att <- df_mldTotal$Price * df_mldTotal$cAtt
df_mldTotal$Price_SN  <- df_mldTotal$Price * df_mldTotal$cSN
df_mldTotal$Price_Segment <- df_mldTotal$Price * df_mldTotal$DSegment
df_mldTotal$ASC_Segment <- df_mldTotal$ASC_accept * df_mldTotal$DSegment
df_mldTotal$ASC_Serv <- df_mldTotal$ASC_accept * df_mldTotal$Servicios


clTotalTPB_socdem <- gmnl(
  Choice ~ 0 + Project + Specie + Location + Price + ASC_accept + Price_Segment + ASC_Att + ASC_SN + ASC_PBC + ASC_Trust + ASC_Risk + ASC_Serv,
  data = df_mldTotal,
  model = "mnl"
)
summary(clTotalTPB_socdem)
```

### Conditional logit: Segment A
```{r}
clAec <- gmnl(
  Choice ~ 0 + Project + Specie + Location + Price + ASC_accept + ASC_Att + ASC_SN + ASC_PBC + ASC_Trust + ASC_Risk + ASC_Serv,
  data = df_mldA,
  model = "mnl"
)
summary(clAec)
...
clAs <- gmnl(
  Choice ~ 0 + Project + Specie + Location + Price + ASC_accept + ASC_Att + ASC_SN + ASC_PBC + ASC_Trust + ASC_Risk + ASC_Serv + ASC_large,
  data = df_mldA,
  model = "mnl"
)
summary(clAs)
...
##### Segment B Halton draws loop halton = list(c(2, 11, 7, 13)) BGFS method
```{r}
...
fit_gmnl_safe <- function(formula, data, ranp, R, corr = FALSE, start = NULL){
  gmnl(formula, data=data, model="mixl", ranp=ranp, panel=TRUE,
       R=R, halton=list(prime = c(2, 11, 7, 13)), correlation=corr,
